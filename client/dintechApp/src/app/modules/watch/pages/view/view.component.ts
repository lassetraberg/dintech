...
export class ViewComponent implements OnInit, AfterViewInit {
  ...
  subscribeToWebsockets() {
    ...
    this.socket = this.ws.getSubject(
      this.sessionId,
  ...
  onPlayerReady() {
    if (this.lateJoinState) {
      ...
      const { offset, isPlaying } = this.lateJoinState.state;
    ...
    if (this.slider) {
      this.subscription = timer(0, 500).subscribe(() => {
        ...
        this.slider.nativeElement.style.background =
          "linear-gradient(to right, rgb(60,58,60) 0%, rgb(60,58,60) " +
          percentage +
          "%, rgb(228,183,153) " +
          percentage +
          "%, rgb(228,183,153) 100%)";
...
class YouTubeVideo {
  constructor(public currentTime?: number, public duration?: number) {}
}
